(* Discretization-specific cut analysis for typed Slice expressions. *)
...
let analyze (e : Inference.typed_expr) : cut_annotated_expr =
  infer_expr (Inference.erase e)

let analyze_bool (e : Inference.typed_expr) : cut_annotated_expr =
  let t, eff, a = analyze e in
