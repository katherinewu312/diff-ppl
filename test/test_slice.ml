...
let transform source =
  ...
  let texpr = infer_with_cuts expr in
...
let forward_gradient source =
  let expr = Slice.Parse.parse_expr source in
  let texpr = infer_with_cuts expr in
  Slice.Forward.gradient texpr

let forward_dual_after_discretize source =
  let transformed = transform source in
  let texpr = infer_with_cuts transformed in
  Slice.Forward.dual_expectation texpr

let forward_dual_after_normalized_discretize source =
  let transformed = transform_normalized source in
  let texpr = infer_with_cuts transformed in
  Slice.Forward.dual_expectation texpr

let forward_dual_raw_after_discretize source =
  let transformed = transform source in
  let texpr = infer_with_cuts transformed in
  Slice.Forward.dual_expectation_raw texpr
...
let forward_dual_after_discretize_at param value source =
  let transformed = transform_at param value source in
  let texpr = infer_with_cuts transformed in
  let raw = Slice.Forward.dual_expectation_raw ~param texpr in
  let simplified = Slice.Forward.dual_expectation ~param texpr in
  ( Slice.Simplify.subst_float param value raw
  , Slice.Simplify.algebraic (Slice.Simplify.subst_float param value simplified) )

let forward_gradient_after_discretize source =
  let transformed = transform source in
  let texpr = infer_with_cuts transformed in
  Slice.Forward.gradient texpr

let forward_dual_with_seeds seeds source =
  let transformed = transform source in
  let texpr = infer_with_cuts transformed in
  Slice.Forward.dual_expectation ~seeds texpr

let seeds assignments =
  List.fold_left
    (fun acc (name, value) -> Slice.Forward.add_seed name value acc)
...
let test_forward_enumerates_direct_discrete_comparison _ =
  let grad = forward_gradient "discrete(theta, 1 - theta) <#2 1#2" in
  assert_equal
    ~printer:string_of_float
    ~cmp:(fun a b -> abs_float (a -. b) < 1e-9)
    1.0
...
let test_forward_includes_probability_and_body_derivatives _ =
  let grad =
    forward_gradient
...
let test_forward_raw_uses_let_bound_forward_rules _ =
  let source =
    "let x = discrete(theta, 1 - theta) in if x <#2 1#2 then theta else 0"
  in
  let raw_plain =
    Slice.Pretty.string_of_expr_plain (forward_dual_raw_after_discretize source)
  in
  assert_bool "raw AD program should bind transformed arithmetic operands"
    (contains_substring raw_plain "_forward_yhat");
  assert_bool "raw AD program should bind transformed discrete probabilities"
    (contains_substring raw_plain "_forward_phat");
  assert_bool "raw AD program should bind transformed discrete continuations"
    (contains_substring raw_plain "_forward_bhat");
  assert_bool "raw AD program should bind weighted discrete arms"
    (contains_substring raw_plain "_forward_chat");
  assert_substring_order raw_plain "_forward_phat" "_forward_bhat";
  assert_substring_order raw_plain "_forward_bhat" "_forward_chat";
  let primal, tangent = eval_dual_with_theta 0.3 (forward_dual_after_discretize source) in
  assert_close 0.09 primal;
  assert_close 0.6 tangent

let test_forward_uniform_cdf_dual_simplifies _ =
  let dual = forward_dual_after_discretize "uniform(0, 1) < theta" in
...
let test_forward_uniform_cdf_gradient_simplifies _ =
  let grad = forward_gradient_after_discretize "uniform(0, 1) < theta" in
  assert_equal
    ~printer:string_of_float
    ~cmp:(fun a b -> abs_float (a -. b) < 1e-9)
    1.0
...
let test_forward_gaussian_cdf_dual_simplifies _ =
  let raw = forward_dual_raw_after_discretize "gaussian(0, 1) < theta" in
  ...
  let dual = forward_dual_after_discretize "gaussian(0, 1) < theta" in
...
let test_forward_uniform_sum_cdf_expr_simplifies _ =
  let dual =
    forward_dual_after_normalized_discretize
...
let test_forward_scaled_uniform_sum_cdf_expr_simplifies _ =
  let dual =
    forward_dual_after_normalized_discretize
...
let test_forward_uniform_product_cdf_expr_simplifies _ =
  let dual =
    forward_dual_after_normalized_discretize
      "uniform(0, 1) * uniform(0, 1) < theta"
  in
...
let test_forward_gaussian_affine_sum_cdf_expr_simplifies _ =
  let dual =
    forward_dual_after_normalized_discretize
...
let test_forward_weighted_gamma_sum_cdf_expr_simplifies _ =
  let dual =
    forward_dual_after_normalized_discretize
...
let test_forward_beta_cdf_dual_simplifies_at _ =
  let raw = forward_dual_raw_after_discretize "beta(2, 3) < theta" in
  ...
  let _, simplified =
    forward_dual_after_discretize_at "theta" 0.4 "beta(2, 3) < theta"
  in
...
let test_forward_dual_at_substitutes_raw_and_simplifies _ =
  let raw, simplified =
    forward_dual_after_discretize_at "theta" 0.3 "uniform(0, 1) < theta"
  in
...
let test_forward_dual_at_can_use_non_theta_parameter _ =
  let _, simplified =
    forward_dual_after_discretize_at "alpha" 0.4 "uniform(0, 1) < alpha"
  in
  match simplified with
  | Slice.Ast.ExprNode
      (Slice.Ast.Pair
...
let test_forward_infers_single_non_theta_seed _ =
  let expr = Slice.Parse.parse_expr "foo * foo" in
  let texpr = infer_with_cuts expr in
  let dual = Slice.Forward.dual_expectation texpr in
...
let test_forward_requires_seed_for_multiple_free_float_variables _ =
...
let test_forward_allows_one_seed_for_multiple_free_float_variables _ =
  let expr = Slice.Parse.parse_expr "foo + bar" in
  let texpr = infer_with_cuts expr in
  let dual = Slice.Forward.dual_expectation ~seeds:(seeds ["foo", 1.0]) texpr in
  let primal, tangent =
    eval_dual_with_env
      [ "foo", Slice.Ast.VFloat 0.4
      ; "bar", Slice.Ast.VFloat 0.3
...
let test_forward_explicit_seed_for_non_theta_variable _ =
  let dual =
    forward_dual_with_seeds
      (seeds ["alpha", 1.0])
      "alpha * alpha + theta"
  in
  let primal, tangent =
...
let test_reverse_discrete_includes_probability_and_body_derivatives _ =
  ...
  let dual = Slice.Reverse.dual_expectation (infer_with_cuts (transform source)) in
  let primal, tangent = eval_dual_with_theta 0.3 dual in
  assert_close 0.09 primal;
  assert_close 0.6 tangent
...
let test_forward_multiple_explicit_unit_seeds _ =
  let dual =
    forward_dual_with_seeds
...
let test_forward_dual_simplifies_polynomial_components _ =
  let source =
    "if discrete(theta, 1 - theta) <#2 1#2 then theta else theta + 1"
  in
  let raw = forward_dual_raw_after_discretize source in
  let raw_plain = Slice.Pretty.string_of_expr_plain raw in
  ...
  match forward_dual_after_discretize source with
  | Slice.Ast.ExprNode
      (Slice.Ast.Pair
         (Slice.Ast.ExprNode (Slice.Ast.Const primal),
          tangent_expr)) ->
...
let test_forward_probabilistic_function_application _ =
  let source =
  ...
  assert_bool "raw probabilistic function should be continuation-passing"
    (contains_substring raw_plain "_forward_k");
  let primal, tangent = eval_dual_with_theta 0.3 (forward_dual_after_discretize source) in
  assert_equal
    ~printer:string_of_float
    ~cmp:(fun a b -> abs_float (a -. b) < 1e-9)
...
let test_forward_probabilistic_function_application_simplifies_at _ =
  let source =
    "let f = fun x -> discrete(x, 1 - x) in if f theta <#2 1#2 then theta else 0"
  in
  let _, simplified = forward_dual_after_discretize_at "theta" 0.5 source in
  match simplified with
  | Slice.Ast.ExprNode
      (Slice.Ast.Pair
         (Slice.Ast.ExprNode (Slice.Ast.Const primal),
...
let test_forward_higher_order_probabilistic_function_application _ =
  let source =
...
let test_forward_probabilistic_fix_loop_application _ =
  let source =
...
let test_forward_probabilistic_fix_loop_application_simplifies_at _ =
  let source =
    "let choose = fix choose x := discrete(x, 1 - x) in \
     if choose theta <#2 1#2 then theta else 0"
  in
...
let test_forward_list_match_simplifies_at _ =
  let source =
...
let test_forward_recursive_list_sum_simplifies_at _ =
  let source =
    "let sum = fix sum xs := \
       match xs with \
       | nil -> 0 \
...
let suite =
  ...
  ; "test_forward_raw_uses_let_bound_forward_rules" >:: test_forward_raw_uses_let_bound_forward_rules
  ; "test_forward_uniform_cdf_dual_simplifies" >:: test_forward_uniform_cdf_dual_simplifies
  ; "test_forward_uniform_cdf_gradient_simplifies" >:: test_forward_uniform_cdf_gradient_simplifies
  ; "test_forward_gaussian_cdf_dual_simplifies" >:: test_forward_gaussian_cdf_dual_simplifies
  ...
  ; "test_forward_uniform_sum_cdf_expr_simplifies" >:: test_forward_uniform_sum_cdf_expr_simplifies
  ; "test_forward_scaled_uniform_sum_cdf_expr_simplifies" >:: test_forward_scaled_uniform_sum_cdf_expr_simplifies
  ; "test_forward_uniform_product_cdf_expr_simplifies" >:: test_forward_uniform_product_cdf_expr_simplifies
  ; "test_forward_gaussian_affine_sum_cdf_expr_simplifies" >:: test_forward_gaussian_affine_sum_cdf_expr_simplifies
  ; "test_forward_weighted_gamma_sum_cdf_expr_simplifies" >:: test_forward_weighted_gamma_sum_cdf_expr_simplifies
  ; "test_forward_beta_cdf_dual_simplifies_at" >:: test_forward_beta_cdf_dual_simplifies_at
  ; "test_forward_dual_at_substitutes_raw_and_simplifies" >:: test_forward_dual_at_substitutes_raw_and_simplifies
  ; "test_forward_dual_at_can_use_non_theta_parameter" >:: test_forward_dual_at_can_use_non_theta_parameter
  ; "test_forward_infers_single_non_theta_seed" >:: test_forward_infers_single_non_theta_seed
  ; "test_forward_requires_seed_for_multiple_free_float_variables" >:: test_forward_requires_seed_for_multiple_free_float_variables
  ; "test_forward_allows_one_seed_for_multiple_free_float_variables" >:: test_forward_allows_one_seed_for_multiple_free_float_variables
  ; "test_forward_explicit_seed_for_non_theta_variable" >:: test_forward_explicit_seed_for_non_theta_variable
  ...
  ; "test_forward_multiple_explicit_unit_seeds" >:: test_forward_multiple_explicit_unit_seeds
  ; "test_forward_dual_simplifies_polynomial_components" >:: test_forward_dual_simplifies_polynomial_components
  ; "test_probabilistic_function_effect_inference" >:: test_probabilistic_function_effect_inference
  ; "test_forward_probabilistic_function_application" >:: test_forward_probabilistic_function_application
  ; "test_forward_probabilistic_function_application_simplifies_at" >:: test_forward_probabilistic_function_application_simplifies_at
  ; "test_forward_higher_order_probabilistic_function_application" >:: test_forward_higher_order_probabilistic_function_application
  ; "test_forward_probabilistic_fix_loop_application" >:: test_forward_probabilistic_fix_loop_application
  ; "test_forward_probabilistic_fix_loop_application_simplifies_at" >:: test_forward_probabilistic_fix_loop_application_simplifies_at
  ; "test_forward_list_match_simplifies_at" >:: test_forward_list_match_simplifies_at
  ; "test_forward_recursive_list_sum_simplifies_at" >:: test_forward_recursive_list_sum_simplifies_at
